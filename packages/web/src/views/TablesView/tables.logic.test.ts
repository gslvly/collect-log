...
describe('create table form logic', () => {
  ...
  it('requires valid options when an enum field is configured during table creation', () => {
    const invalid = validateCreateTableForm({
      displayName: '枚举表',
      description: '',
      fields: [{ ...sampleField, type: 'enum', options: [] }],
    });
    expect(invalid).toMatchObject({
      valid: false,
      fields: [{ options: expect.any(String) }],
    });

    const options = [{ value: 'sms', label: '短信', status: 'active' as const }];
    const input = toCreateTableInput({
      displayName: '枚举表',
      description: '',
      fields: [{ ...sampleField, type: 'enum', options }],
    });
    expect(input.fields[0]?.options).toEqual(options);
    expect(input.fields[0]?.options).not.toBe(options);
  });
